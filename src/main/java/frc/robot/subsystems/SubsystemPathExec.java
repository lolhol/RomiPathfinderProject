...
public class SubsystemPathExec extends SubsystemBase {
   ...
   * }
   * 
   * if (curGrid.getDistanceBetweenPosAndLoc(
   * new int[] { curSQGoing.x, curSQGoing.y }) < 10) {
   * curSQGoing = null;
   * 
   ...
   * 
   * double dy = 0 - posMMNode[1]; // 7228-7450
   * double dx = posMMNode[0] - 0; // 5000-5313
   * double angle = MathUtil.normaliseDeg(
   * Math.atan2(dy, dx) / Math.PI * 180);
   * 
   * return MathUtil.diffDeg(curPosDataMMDeg[2], angle);
   * }
   * 
